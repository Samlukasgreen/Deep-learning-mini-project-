...
OUT_DIR      = r"C:\Users\Eladio\Documents\Intro to Deep Learning\BlendedNet Dataset - Released\clarc_blended_wing_body-main\runs"
BATCH_SIZE   = 48
EPOCHS       = 10000
LR           = 1e-2
LATENT       = 128
...
def train_one_epoch(model, loader, optim, criterion, mu, std, accum_steps=1):
    model.train()
    total = 0.0
    n = 0
    ...
    #optim.zero_grad()                          # zero once, before the loop

    for i, (pts, y, _, _) in enumerate(loader):
        ...
        pred_n = model(pts)
        t_loss  += criterion(pred_n, y_n)

        # Scale so the accumulated gradient equals the mean over accum_steps batches

        total = t_loss.item() * pts.size(0)
        n     += pts.size(0)
    ...
    # Flush any leftover batches (if len(loader) isn't divisible by accum_steps)
    if (i + 1) % accum_steps != 0:
        optim.step()
        optim.zero_grad()
...
def main():
    ...
    dtrain, dval = split_by_geom(dtrain_full, val_frac=0.3)

    # Loaders
    dumb= DataLoader(dtrain_full, batch_size=BATCH_SIZE, shuffle=True,  num_workers=0, pin_memory=True)
    ...
    test_loader  = DataLoader(dtest,  batch_size=BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=True)

    # Target normalization (z-score) computed on TRAIN ONLY
    mu, std = compute_target_norm_stats(dumb)
    ...
    print(f"[data] total={len(dtrain_full)}  train={len(dtrain)}  val={len(dval)}")
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        print("changed:", (before - after).abs().sum().item())
        print(
            f"[Epoch {epoch:04d}] "
            f"train_mse={tr:.6e} val_mse={va:.6e} "
            f"time={(time.time() - t1):.1f}s"
        )
        if epoch % 10 == 0 or epoch == 1:
            print(f"[{epoch:04d}] train {tr:.5f} | val {va:.5f}")
    
    print(f'Total time: {(time.time() - t0)/60:.1f}')
    return
