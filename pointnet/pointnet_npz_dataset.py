...
def canonical_geom_name(name: str) -> str:
    """
    Normalize geometry naming variants to CSV style: geom_###
    Examples:
      - geom_000 -> geom_000
      - case_000 -> geom_000
      - 000      -> geom_000
    ...
    if "_" in s:
        ...
        if suffix.isdigit():
            return f"geom_{int(suffix):03d}"
        return s
    if s.isdigit():
        return f"geom_{int(s):03d}"
    return s
...
class NPZPointParamDataset(Dataset):
    ...
    def __init__(self, shards_dir: str, params_csv: str, target_cols=TARGET_COLS, transform=None):
        ...
        self.targets_map = {canonical_geom_name(row["geom_name"]): row[target_cols].to_numpy(dtype=np.float32)
        ...
        for f in self.files:
            with np.load(f) as data:
                if "geom_name" in data:
                    raw_geom = str(data["geom_name"])
                elif "geo_name" in data:
                    raw_geom = str(data["geo_name"])
                else:
                    raw_geom = os.path.splitext(os.path.basename(f))[0]
                geom = canonical_geom_name(raw_geom)
            if geom in self.targets_map:
                keep.append(f)
            else:
                self.missing.append((os.path.basename(f), raw_geom, geom))
        self.files = keep
        if len(self.files) == 0:
            sample = ", ".join([f"{fn}:{raw}->{canon}" for fn, raw, canon in self.missing[:5]])
            raise RuntimeError(
                "After filtering, no shards matched the CSV's geom_name. "
                f"Sample shard names: {sample}"
            )
    ...
    def __getitem__(self, idx):
        ...
        # to (3,N) for PointNet
        pts = torch.from_numpy(xyz.T.copy())[:,:8192]  # (3,N)
        if "geom_name" in data:
            raw_geom = str(data["geom_name"])
        elif "geo_name" in data:
            raw_geom = str(data["geo_name"])
        else:
            raw_geom = os.path.splitext(os.path.basename(path))[0]
        geom = canonical_geom_name(raw_geom)
        y = torch.from_numpy(self.targets_map[geom])  # (9,)
        #print('#############(pts.shape, y.shape, geom, os.path.basename(path))####################')
        #print(pts.shape, y.shape, geom, os.path.basename(path))
